<template>
  <n-form-item :label="label" :required="isRequired" :path="fieldName" :feedback="feedback">
    <n-checkbox-group v-model:value="modelValue" :name="fieldName" :id="fieldName" :disabled="!isEditable">
      <div class="checkbox-option" :class="{ inline: !vertical }" v-for="option in optionsList" :key="option.value">
        <n-checkbox :value="option.value" :disabled="isEditable && disabledOptions.includes(option.value)">
  ...
  <n-modal v-model:show="showModal" preset="card" style="max-width: 800px" :title="descriptionTitle" size="small">

<template>
  <radio-input
    ...
    :options="pagmanieroj"
    vertical
    :remarks="{ Stripe: 'Ni rekomendas uzi tiun ĉi pagmanieron ĉar tio ebligos al UEA plej rapide trakti la aliĝon.' }"
  />
  <slide-transition>
    <uea-code-input
  ...
  <slide-transition>
    <n-p v-if="form.pagmaniero !== null && form.pagmaniero.startsWith('Peranto:')">
      Bazaj informoj pri via aliĝo estos senditaj ankaŭ al la peranto.
    </n-p>
  </slide-transition>
  <slide-transition>
    <n-card title="Pagformularo" class="custom-card" v-show="form.pagmaniero === 'Stripe'">
      ...
      <n-grid x-gap="12" :cols="2" style="margin-top: 1.5rem">
        <n-gi>
          <text-input v-model="form.karto_retadreso" field-name="karto_retadreso" type="email" />
        </n-gi>
        <n-gi>
          <text-input v-model="form.karto_posedanto" field-name="karto_posedanto" />
        </n-gi>
      </n-grid>
      <p>
        Pagformularo provizita de <a href="https://stripe.com/" target="_blank">Stripe</a>. Kartdatumoj ne estas iel ajn
        konservataj de nia sistemo, nek videblas al ni.
      </p>
    </n-card>
  </slide-transition>
</template>
...
const card_errors = ref(undefined);
const server_endpoint = axios.defaults.baseURL + '/saveform';
const stripe = {
...
const pagmanieroj = computed(() => {
  const payments = { ...formOptions.value.payment };

  if (form.value.lando in formOptions.value.perantoj) {
    // Turn object into array
    const options = [];
    for (const key in payments) {
      if (!(!props.ueaCodeValid && key === 'UEA')) {
        options.push({
          value: key,
          label: payments[key],
        });
    ...
    // Insert perantoj
    formOptions.value.perantoj[form.value.lando].forEach((peranto) => {
      options.splice(-2, 0, {
        value: 'Peranto: ' + peranto.uea_kodo,
        label: 'Pere de kongresa peranto en ' + countries.value[form.value.lando].name + ': ' + peranto.nomo,
      });
    });
...
function on_card_change(d) {
  console.log('on_card_change:', d);
  card_errors.value = d.error ? d.error.message : '';
}
</script>
